...
def download_data(base_date, day_offset):
    ...
    QUERY = f"""
        SELECT siteid, sitename, IBCCustId, Currency, doing, logdate, IP, Status, ActionType, OS, OS_version, browser, browser_version, Platform, Action, LoginUrl
        FROM `nf-bifrost.KeyIndex.user_login` 
        WHERE logdate BETWEEN DATETIME("{current_date.strftime('%Y-%m-%d')}") 
    ...
    filename = os.path.join(
        os.getenv("HOME"), f'work/behavior/login_{current_date.strftime("%Y%m%d")}.csv'
    )
