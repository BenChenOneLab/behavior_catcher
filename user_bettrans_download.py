...
def download_data(base_date, day_offset):
    ...
    QUERY = f"""
        SELECT custid as cust_id, status, transdate as date, betfrom 
        FROM `behavior-catcher.behavior_catcher_dm.bettrans_202406` 
        WHERE transdate BETWEEN DATETIME("{current_date.strftime('%Y-%m-%d')}") 
        AND DATETIME("{next_date.strftime('%Y-%m-%d')}") 
        AND betfrom in ("W003", "W004")
    """
    ...
    filename = os.path.join(
        os.getenv("HOME"),
        f'work/behavior/bettrans_{current_date.strftime("%Y%m%d")}.csv',
    )
